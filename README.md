...
put these calls in the job's `.ps1`. `$cfg` is a hashtable supplied by that script. load JSON if useful, then add runtime values such as credentials. [settings.json](examples/settings.json) and [job.ps1](examples/job.ps1) show SQL + CSV + email.
