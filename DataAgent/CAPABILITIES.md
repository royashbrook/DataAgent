...
runtime values: any string that is exactly `env:NAME`, at any depth, is replaced with that environment variable when the run starts. an unset or empty one stops the run and the error names the variable. an object with exactly the keys `username` and `password` (after that replacement) becomes a PSCredential. the config names every variable, so the runner assumes none of its own. a settings file on disk is read, never rewritten, and a relative `directory` in it is relative to the file. arrays and script-supplied lists are both walked. objects other than hashtables and arrays, such as a PSCredential built by the caller, pass through unchanged. ANSI colour is kept to the screen for the run and the prior setting is restored after.
